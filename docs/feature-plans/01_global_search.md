...
- **View Mode:** Products must display in **List Mode** by default (not Grid Mode).
...
| **TC_SEARCH_005** | Verify edge case handling of whitespace-only queries.                                 | User is on the application Home Page. | 1. Click into the search input field.<br>2. Type a string of empty spaces: `'     '`.<br>3. Press the **Enter** key on the keyboard.          | 1. Browser redirects to the Search Results page.<br>2. No products are displayed.<br>3. The message: `"There is no product that matches the search criteria."` is displayed.        | **Regression Test** |
